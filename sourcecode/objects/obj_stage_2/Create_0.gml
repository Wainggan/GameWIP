	...
	command_set([
		...
		function(){
			b_golden = bullet_preset_golden(x, y, 0, 5, b_golden + 2, function(_x, _y, _dir, i) {
				_dir *= b_dir
				with bullet_shoot_dir3(_x, _y, 8, 0.2, 1, 0.05, i % 6 == 0 ? -b_speed * 1.45 : b_speed * 2, _dir) {
...
pattern_add("stage2-boss-3", function() {
			...
			bullet_preset_ring(WIDTH / 2, HEIGHT / 2, 10 + (b_count++ % 4), point_distance(0, 0, WIDTH, HEIGHT)/2 + 16, frac(sin(time_phase / 60) * 1000) * 360, function(_x, _y, _dir) {
				...
				with bullet_shoot(_x, _y) {
					...
					bullet_set_step(, function(){
						var _c = b_calc(x, y, b_other.time_phase / 60)
						spd = (abs(_c - 0.5) * -2 + 1) * 1 + 3
						dir = _c * 360 + 90
					})
				}
			})
	...
	command_add([
		...
		20,
		function(){
			bullet_preset_plate(x, y, 7, 12, 40, -16, b_dir, function(_x, _y, _dir){
				with bullet_shoot_dir2(_x, _y, 0.1, 0.01, 1.5, _dir) {
					bullet_set_look(, spr_bullet_normal, cb_rust)
				}
			})
			command_repeat(3)
		},
