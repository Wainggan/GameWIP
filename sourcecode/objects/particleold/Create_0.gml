...
_burst = function(xp, yp, xv, yv, _name) {
	switch _name {
		case "bulletExplosion":
			if xv != undefined {
				var d = point_direction(0, 0, xv, yv)
				part_type_direction(bulletExplode2, d - 120, d + 120, 0, 0);

			}
			part_emitter_region(particleSystem, bulletExplode2_PE, xp+0, xp+0, yp+0, yp+0, ps_shape_rectangle, ps_distr_linear);
			part_emitter_burst(particleSystem, bulletExplode2_PE, bulletExplode2, 12);
			part_emitter_region(particleSystem, bulletExplode1_PE, xp+0, xp+0, yp+0, yp+0, ps_shape_rectangle, ps_distr_linear);
...
burst = function(_x, _y, _name, _xv, _yv) {
	_burst(_x, _y, _xv, _yv, _name);
}